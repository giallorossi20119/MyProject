...
#include "CapsaBantingGameScene.h"
...
//*******************************
int g_iDistance = 0;
void CapsaBantingGameScene::onEnter()
{
	BaseScene::onEnter();
    schedule(schedule_selector(CapsaBantingGameScene::update));
	//load scene
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if(btnInvite)
        {
            btnInvite->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnInviteClick, this));
        }
    ...
    pnlPlayerAreas[0]->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::unselectedCards, this));
    ...
    btnPlace->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnPlaceClick, this));
    btnPass          = static_cast<Button*>(Helper::seekWidgetByName(m_pLayout, "btnPass"));
    btnPass->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnPassClick, this));
    btnSort            = static_cast<Button*>(Helper::seekWidgetByName(m_pLayout, "btnSort"));
    btnSort->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnSortClick, this));

    pnlTableArea->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::unselectedCards, this));
...
//    GlobalVariable::m_pSettingCom->setPosition(Vec2(-700, -430));
    ...
    btnStart->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnStartClick, this));
	...
	m_btnStraight->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnStraight, this));
	m_btnFlush->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnFlush, this));
	m_btnFullhouse->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnFullhouse, this));
	m_btnFourOfAKind->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnFourOfAKind, this));
	m_btnStraightFlush->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnStraightFlush, this));
	m_btnRoyalStraightFlush->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onTouchBtnRoyalStraightFlush, this));
    ...
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onSelectedCard), "select card", nullptr);
...
void CapsaBantingGameScene::spectator()
{
...
void CapsaBantingGameScene::reconnect()
{
    sortCards();
...
void CapsaBantingGameScene::unselectedCards(Ref *pSender, Widget::TouchEventType type)
{
...
void CapsaBantingGameScene::onCountDownTimer(float dt)
{
    ...
    if(m_fCountTimeLeft < 0)
    {
        m_bmfCountDown->setVisible(false);
        m_pnlCountDown->setVisible(false);
        unschedule(schedule_selector(CapsaBantingGameScene::onCountDownTimer));
        return;
    }
...
void CapsaBantingGameScene::onPrgTimer(float dt)
{
    ...
    if(m_fCountTimeLeft < 0)
    {
        ...
        unschedule(schedule_selector(CapsaBantingGameScene::onPrgTimer));
        return;
    }
...
void CapsaBantingGameScene::onResetGame(Ref* obj)
{
    m_pnlCountDown->setVisible(false);
    m_bmfCountDown->setVisible(false);
...
void CapsaBantingGameScene::onSelectedCard(Ref* obj)
{
	Card* card = (Card*)obj;
...
void CapsaBantingGameScene::setTurnId(double p_iTurnId, bool p_bFirst)
{
    if(!GlobalVariable::m_cSetData->m_bSpectator)
    {
    ...
    if(p_iTurnId == GlobalVariable::m_pUser->m_dId && !GlobalVariable::m_cSetData->m_bSpectator)
    {
        ...
        unschedule(schedule_selector(CapsaBantingGameScene::onPrgTimer));
        schedule(schedule_selector(CapsaBantingGameScene::onPrgTimer), 0.0f, kRepeatForever, 0.0f);
        //disable other turn
...
void CapsaBantingGameScene::placePlayers()
{
    for(int i = 0; i < 4; i++)
    {
...
void CapsaBantingGameScene::showInvite()
{
    for (int i = 1; i < 4; i++)
    {
...
void CapsaBantingGameScene::showKickUser(bool bShow)
{
    for(int i = 0; i < m_vUserInfoOthers->size(); i++)
    {
...
void CapsaBantingGameScene::showHostUser()
{
    for(int i= 0; i < GlobalVariable::m_vPlayersSorted->size(); i++)
    {
...
void CapsaBantingGameScene::showItemsRoom()
{
    if(GlobalVariable::m_pUser->m_bHost && !GlobalVariable::m_cSetData->m_bPlaying)
    {
        showKickUser(true);
...
void CapsaBantingGameScene::setActiveButtons(double iTurnID, bool bFirst)
{
...
//start game
void CapsaBantingGameScene::onBtnStartClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onBtnPlaceClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onBtnSortClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onBtnPassClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
    AudioManager::getInstance()->playEffect(kSoundEffectButtonClick);
...
void CapsaBantingGameScene::onBtnInviteClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
    AudioManager::getInstance()->playEffect(kSoundEffectButtonClick);
...
void CapsaBantingGameScene::update(float dt)
{
	BaseScene::update(dt);
}

void CapsaBantingGameScene::relayout()
{
	Size deviceSize = Director::getInstance()->getWinSize();
...
void CapsaBantingGameScene::addEventlistener()
{
	// Add eventlistener
	//Button* btnBack = static_cast<Button*>(Helper::seekWidgetByName(m_pLayout, "btnBack"));
	//btnBack->addTouchEventListener(CC_CALLBACK_2(CapsaBantingGameScene::onBtnBackClick, this));
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onUserJoinRoom), C_CLIENT_USER_JOIN_ROOM, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onUserLeftRoom), C_CLIENT_USER_OUT_ROOM, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onStartedGame), SC_CLIENT_START_GAME, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onFireCard), SC_CLIENT_DANH_BAI, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onIgnoreCard), SC_CLIENT_BO_LUOT, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onChangeRound), C_CLIENT_CHANGE_ROUND, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onClientGetStart), C_CLIENT_STARTID, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onClientFinishGame), C_CLIENT_FINISH, nullptr);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onNetworkSendPublishMsr), EVENT_NETWORK_SEND_MSR, nullptr);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onNetworkGetPublishMsr), EVENT_NETWORK_GET_MSR, nullptr);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::showSettingRoom), EVENT_SHOW_SETTING_ROOM, nullptr);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onNetworkAutoFire), SC_COMMAND_AUTO, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onKillDead), C_COMMAND_CHATCHEM, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onWaitingList), C_COMMAND_GET_LISTWAITING, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onDead2), C_CLIENT_DEAD_2, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onResetGame), C_CLIENT_RESET_GAME, nullptr);
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onEndShowResult), C_CLIENT_END_SHOW_RESULT, nullptr);
	NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaBantingGameScene::onRoomSettingChanged), S_CLIENT_CHANGE_SETTING_GAME, nullptr);
	//keyboard event
	auto m_listenerKeyboard = EventListenerKeyboard::create();
	m_listenerKeyboard->onKeyReleased = CC_CALLBACK_2(CapsaBantingGameScene::onKeyReleased, this);
	getEventDispatcher()->addEventListenerWithFixedPriority(m_listenerKeyboard, 1);
}

void CapsaBantingGameScene::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
...
void CapsaBantingGameScene::onWaitingList(Ref* obj)
{
...
void CapsaBantingGameScene::onKillDead(Ref* obj)
{
    ...
    for(int j = 0; j < GlobalVariable::m_vPlayersSorted->size(); j++)
    {
        if(cmdKillData->m_dKillId == GlobalVariable::m_vPlayersSorted->at(j)->m_dId)
        {
            ...
            auto actFinishWin = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishAction, this, bmfMoneyWin));
        ...
        if(cmdKillData->m_dDeadId == GlobalVariable::m_vPlayersSorted->at(j)->m_dId)
        {
            ...
            auto actFinisLose = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishAction, this, bmfMoneyLose));
...
void CapsaBantingGameScene::onDead2(Ref* obj)
{
...
void CapsaBantingGameScene::onFireCard(Ref* obj)
{
    ...
    //display card on table
    int aPos[5][2] = {{kDISTANCE_X_CARD_ON_TABLE, kDISTANCE_Y_CARD_ON_TABLE},
...
void CapsaBantingGameScene::onStartedGame(Ref* obj)
{
    unschedule(schedule_selector(CapsaBantingGameScene::onCountDownTimer));
    showMessage(false, "");
    m_iCountTurns = 0;
...
void CapsaBantingGameScene::onClientGetStart(Ref* obj)
{
...
void CapsaBantingGameScene::setVisibleGameButtons(bool bVisible)
{
...
void CapsaBantingGameScene::onIgnoreCard(Ref* obj)
{
    Size deviceSize = Director::getInstance()->getWinSize();
    CMDIgnoreCardData* cmdIgnoreCardData = (CMDIgnoreCardData*)obj;
    for(int j = 0; j < GlobalVariable::m_vPlayersSorted->size(); j++)
...
void CapsaBantingGameScene::onChangeRound(Ref* obj)
{
    clearCards();
    m_iCountTurns = 0;
    m_iHandCount = 0;
...
void CapsaBantingGameScene::onNetworkAutoFire(Ref* obj)
{
    bool bAuto = (bool)obj;
}

void CapsaBantingGameScene::displayCards()
{
...
void CapsaBantingGameScene::dealCards()
{
    Size deviceSize = Director::getInstance()->getWinSize();
    ...
    for(int i = 0; i < 13; i++)
    {
        for(int j = 0; j < GlobalVariable::m_vPlayersSorted->size(); j++)
        {
            ...
            {
                auto act4 = CallFunc::create(CC_CALLBACK_0(CapsaBantingGameScene::animFinishMe, this, card, i));
                moveto = MoveTo::create(0.05f, Vec2(x + i * g_iDistance, -100));
                actions.pushBack(act4);
            }
            else
            {
                auto act5 = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::animFinishOther, this, card, j, i));
    ...
    auto act6 = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishAction, this, card));
...
void CapsaBantingGameScene::finishAction(Node* node)
{
    node->removeFromParent();
}

void CapsaBantingGameScene::finishWinAction()
{
    if(GlobalVariable::m_pUser->m_bPlaying)
    {
        GlobalVariable::m_cSetData->m_bSpectator = false;
    ...
    unschedule(schedule_selector(CapsaBantingGameScene::onPrgTimer));
    unschedule(schedule_selector(CapsaBantingGameScene::onCountDownTimer));
    schedule(schedule_selector(CapsaBantingGameScene::onCountDownTimer), 1.0f, kRepeatForever, 0.0f);
...
void CapsaBantingGameScene::animFinishOther(Node* node, int indexUser, int indexCard)
{
    m_vUserInfoOthers->at(indexUser)->updateNOCardsRemain(indexCard + 1);
}

void CapsaBantingGameScene::animFinishMe(Node* node, int indexCard)
{
    Size deviceSize = Director::getInstance()->getWinSize();
    int x = (pnlPlayerAreas[0]->getContentSize().width - 12 * g_iDistance) / 2;
...
void CapsaBantingGameScene::clearCards()
{
    pnlTableArea->removeAllChildren();
}

void CapsaBantingGameScene::onClientError(Ref* obj)
{
...
void CapsaBantingGameScene::sortCards()
{
    if(m_iTypeSort == 0)
    {
...
void CapsaBantingGameScene::onUserJoinRoom(Ref* obj)
{
...
void CapsaBantingGameScene::joinRoom()
{
    placePlayers();
...
void CapsaBantingGameScene::onUserLeftRoom(Ref* obj)
{
...
void CapsaBantingGameScene::leftRoom(double iTTID)
{
    //if you goto lobby
...
void CapsaBantingGameScene::finishGame(vector<CMDFinishData*>* vCMDFinishData)
{
    auto deviceSize = Director::getInstance()->getWinSize();
    pnlTableArea->removeAllChildren();
            ...
            if(vCMDFinishData->at(i)->m_dId == GlobalVariable::m_vPlayersSorted->at(j)->m_dId)
            {
                ...
                if(vCMDFinishData->at(i)->m_bwin)
                {
                       ...
                       || vCMDFinishData->at(i)->m_sRank == FN_RONG_DONG_HOA)
                    {
                        ...
                        auto actFinish = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishAction, this, bmfType));
                ...
                if(index == 0)//you win
                {
                    ...
                    auto actFinish = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishAction, this, txtBMMoney));
...
void CapsaBantingGameScene::onEndShowResult(Ref* obj)
{
    finishWinAction();
}

void CapsaBantingGameScene::onClientFinishGame(Ref* obj)
{
    m_bFinishGame = true;
    m_hintArrow->setVisible(false);
    pnlGameBar->setVisible(false);
    ...
    auto actFinishGame = CallFunc::create( CC_CALLBACK_0(CapsaBantingGameScene::finishGame, this, vCMDFinishData));
    auto seq = Sequence::create(delayAct, actFinishGame, NULL);
    runAction(seq);
}

void CapsaBantingGameScene::changeLanguage(Ref* obj)
{
...
void CapsaBantingGameScene::onRoomSettingChanged(Ref* obj)
{
...
void CapsaBantingGameScene::showSettingRoom(Ref* obj)
{
...
void CapsaBantingGameScene::showMessage(bool bVisible, const char* messageid)
{
...
void CapsaBantingGameScene::onBtnChangeLanguageClick(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
    if (GlobalVariable::m_iCurrentLanguage == LANGUAGE_ENGLISH)
    {
        ResourceManager::getInstance()->setLanguage(LANGUAGE_INDONESIA);
    }
    else if (GlobalVariable::m_iCurrentLanguage == LANGUAGE_INDONESIA)
    {
        ResourceManager::getInstance()->setLanguage(LANGUAGE_ENGLISH);
    }
    TextField* txtLabel = static_cast<TextField*>(Helper::seekWidgetByName(m_pLayout, "txtLabel"));
    NotificationCenter::getInstance()->postNotification(EVENT_LANGUAGE_CHANGE, (Ref*)(intptr_t)txtLabel);
...
//*******************************
void CapsaBantingGameScene::onExit()
{
	BaseScene::onExit();
    ...
    unschedule(schedule_selector(CapsaBantingGameScene::onCountDownTimer));
    unschedule(schedule_selector(CapsaBantingGameScene::update));
    unschedule(schedule_selector(CapsaBantingGameScene::onPrgTimer));
}

/* end pragma exit Scene */

CapsaBantingGameScene::CapsaBantingGameScene()
{
...
void CapsaBantingGameScene::onNetworkSendPublishMsr(Ref* obj)
{
...
void CapsaBantingGameScene::onNetworkGetPublishMsr(Ref* obj)
{
...
void CapsaBantingGameScene::activateHintHands()
{
    ...
    m_iIndexRoyalStraightFlushHand = -1;
/*
    m_btnStraight->setOpacity               (m_vStraightHand.size()		> 0 ? 255 : 122);
    m_btnFlush->setOpacity                  (m_vFlushHand.size()         > 0 ? 255 : 122);
    m_btnFullhouse->setOpacity              (m_vFullhouseHand.size()     > 0 ? 255 : 122);
    m_btnFourOfAKind->setOpacity            (m_vFourOfAKindHand.size()   > 0 ? 255 : 122);
    m_btnStraightFlush->setOpacity          (m_vStraightFlushHand.size() > 0 ? 255 : 122);
    m_btnRoyalStraightFlush->setOpacity     (m_vRoyalStraightFlushHand.size() > 0 ? 255 : 122);

    m_btnStraight->setTouchEnabled          (m_vStraightHand.size()		> 0 ? true : false);
    m_btnFlush->setTouchEnabled             (m_vFlushHand.size()         > 0 ? true : false);
    m_btnFullhouse->setTouchEnabled         (m_vFullhouseHand.size()     > 0 ? true : false);
    m_btnFourOfAKind->setTouchEnabled       (m_vFourOfAKindHand.size()   > 0 ? true : false);
    m_btnStraightFlush->setTouchEnabled     (m_vStraightFlushHand.size() > 0 ? true : false);
    m_btnRoyalStraightFlush->setTouchEnabled(m_vRoyalStraightFlushHand.size() > 0 ? true : false);
*/
...
void CapsaBantingGameScene::inactivateHintHands()
{
    /*
    m_btnStraight->setOpacity(122);
    m_btnFlush->setOpacity(122);
    m_btnFullhouse->setOpacity(122);
    m_btnFourOfAKind->setOpacity(122);
    m_btnStraightFlush->setOpacity(122);
    m_btnRoyalStraightFlush->setOpacity(122);

    m_btnStraight->setTouchEnabled(false);
    m_btnFlush->setTouchEnabled(false);
    m_btnFullhouse->setTouchEnabled(false);
    m_btnFourOfAKind->setTouchEnabled(false);
    m_btnStraightFlush->setTouchEnabled(false);
    m_btnRoyalStraightFlush->setTouchEnabled(false);
    */
...
void CapsaBantingGameScene::deleteHintHands()
{
...
/* Sort by hand in deck                                      */
/*************************************************************/
void CapsaBantingGameScene::onTouchBtnStraight(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onTouchBtnFlush(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onTouchBtnFullhouse(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onTouchBtnFourOfAKind(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onTouchBtnStraightFlush(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type) return;
...
void CapsaBantingGameScene::onTouchBtnRoyalStraightFlush(Ref *pSender, Widget::TouchEventType type)
{
    if (Widget::TouchEventType::ENDED != type) return;
