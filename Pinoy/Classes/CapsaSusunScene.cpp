...
{
    BaseScene::onEnter();
    GlobalVariable::m_vCMDFinishData = nullptr;
    // Game loop event
    ...
    if(!GlobalVariable::m_bGameRestarted)
    {
        ...
        if(!(GlobalVariable::m_startGameData->m_iNOPlayers == 1))
        {
            ...
            {
                if (GlobalVariable::m_startGameData->m_iNOPlayers == 4)
                {
...
//        if(GlobalVariable::m_pUser->m_bHost==false)
//        {
...
//        }
    ...
    if(!GlobalVariable::m_startGameData->m_bPlaying)
    {
        ...
        {
            SettingRoomPopup* pu = new SettingRoomPopup(GlobalVariable::m_pDataMinMaxStake, (int)GlobalVariable::m_cSetData->m_iCuoc, (int)GlobalVariable::m_cSetData->m_iTime, true);
            openPopup(pu);
        }
    }
}
...
void CapsaSusunScene::addNotification()
{
    ...
    //bo sung check them mau binh
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(CapsaSusunScene::onMaubinhCheck), SS_R_MAU_BINH, nullptr);
...
void CapsaSusunScene::onMaubinhCheck(cocos2d::Ref *obj)
{
    SSCMDMaubinhData* maubinhdata = (SSCMDMaubinhData*)obj;
    GlobalVariable::m_bMaubinhCheck=true;
}
...
//                        if (data->m_vSoChi.at(0)==1&&data->m_vSoChi.at(1)==1&&data->m_vSoChi.at(2)==1)
//                        {
//                            CCLOG("show scoop");
//                            showWinAll(2,iPlayerArea);
//                        }
//                        if (data->m_vSoChi.at(0)==0&&data->m_vSoChi.at(1)==0&&data->m_vSoChi.at(2)==0)
//                        {
//                            showWinAll(1,iPlayerArea);
//                        }
                    }
                }
            }
...
{
    GlobalVariable::m_bMaubinhCheck=false;
    GlobalVariable::m_bDontShowMsg=false;
...
void CapsaSusunScene::onTouchBtnFinish(Ref* sender, Widget::TouchEventType type)
{
    ...
    if (GlobalVariable::m_bMaubinhCheck==true)
    {
        m_btnChange->setVisible(false);
    }
    else
...
void CapsaSusunScene::onShowEndGameResult()
{
    ...
    if(GlobalVariable::m_bSusunWin)
    {
        AudioManager::getInstance()->playEffect(kSoundMusicWin);
        GlobalVariable::m_bSusunWin=false;
    }
...
void CapsaSusunScene::showWinAll(int iType,int iPlayerIndex)
{
    Size deviceSize = Director::getInstance()->getWinSize();
    std::string strWin = "PUSOY";
    std::string strLose = "SCOOP";
    TextBMFont* bmFont = TextBMFont::create(iType == 1 ? strLose : strWin, iType == 1 ? "bf0-export.fnt" : "bf2-export.fnt");
    Vec2 pos = m_v_vCardAnimPos.at(iPlayerIndex).at(10);
    bmFont->setAnchorPoint(Vec2(0.5f, 0.5f));
    bmFont->setPosition(pos);
    bmFont->setZOrder(4);
    ...
    bmFont->setOpacity(0);
    bmFont->setScale(2.0f);
//    bmFont->runAction(Sequence::create(
//                                       FadeTo::create(20.0f, 255),
...
void CapsaSusunScene::showChiResult(int iType, int iPlayerIndex, int iHandIndex)
{
    ...
    bmFont->runAction(Sequence::create(
                                       ...
                                       CallFunc::create(CC_CALLBACK_0(CapsaSusunScene::onDeleteNode, this, bmFont)),
                                       nullptr));
    AudioManager::getInstance()->playEffect(kSoundSochi);
}
