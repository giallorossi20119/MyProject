...
const int kRank[] = {
	TongitsCard::RANK_2, TongitsCard::RANK_3, TongitsCard::RANK_4, TongitsCard::RANK_5, TongitsCard::RANK_6, TongitsCard::RANK_7, TongitsCard::RANK_8, TongitsCard::RANK_9, TongitsCard::RANK_10, TongitsCard::RANK_JACK, TongitsCard::RANK_QUEEN, TongitsCard::RANK_KING, TongitsCard::RANK_ACE
};
const int kSuit[] = {
TongitsCard::SUIT_CLUB, TongitsCard::SUIT_SPADE, TongitsCard::SUIT_HEART, 	TongitsCard::SUIT_DIAMOND
};
...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
int aCombinationIndexTong[100];

bool sortIncreaseTong(int a, int b)
{
	return (a < b) ? true : false;
}

vector<int> getCombinationTong(vector<int> &group, int m)
{
	vector<int> result = {};
	for (int i = 1; i <= m; i++)
	{
		result.push_back(group.at(aCombinationIndexTong[i] - 1));
	}
	return result;
}

void findCombinationTong(int i, vector<int> &group, int m, vector<vector<int>> &result)
{
	int n = group.size();
	for (int j = aCombinationIndexTong[i - 1] + 1; j <= n - m + i; j++)
	{
		aCombinationIndexTong[i] = j;
		if (i == m) result.push_back(getCombinationTong(group, m));
		else findCombinationTong(i + 1, group, m, result);
	}
}

vector<vector<int>> getStraightCardsFromRanksTong(vector<int> vFoundRanks, vector<int> allCardIds)
{
	...
	for (int i = 0, l = vFoundRanks.size(); i < l; i++)
	{
		...
		vector<int> foundCards = CardTongHelper::getCardsWithSameRank(rank, allCardIds);
...
vector<vector<int>> CardTongHelper::getCombinations(int noElementRequired, vector<int> group)
{
	aCombinationIndexTong[0] = 0;
	vector<vector<int>> vFoundCombination = {};
	findCombinationTong(1, group, noElementRequired, vFoundCombination);
	return vFoundCombination;
}

int CardTongHelper::getCard(int rank, int suit)
{
	int ind = (rank - 2) * 4 + suit - 1;
	return ind;
}
int CardTongHelper::getRank(int iCardId)
{
	int rankInd = (((iCardId) / 4) >> 0);
	return kRank[rankInd];
}
int CardTongHelper::getSuit(int iCardId)
{
	int suitInd = (iCardId) % 4;
	return kSuit[suitInd];
}

int CardTongHelper::getStrongestCardWithRank(int rank, vector<int> cardIds)
{
	vector<int> cardsWithSameRank = getCardsWithSameRank(rank, cardIds);
	if (cardsWithSameRank.size() == 0) return -1;
	std::sort(cardsWithSameRank.begin(), cardsWithSameRank.end(), sortIncreaseTong);
	return cardsWithSameRank.at(cardsWithSameRank.size() - 1);
}
int CardTongHelper::getStrongestCardWithSuit(int suit, vector<int> cardIds)
{
	vector<int> cardsWithSameSuit = getCardsWithSameSuit(suit, cardIds);
	if (cardsWithSameSuit.size() == 0) return -1;
	std::sort(cardsWithSameSuit.begin(), cardsWithSameSuit.end(), sortIncreaseTong);
	return cardsWithSameSuit.at(cardsWithSameSuit.size() - 1);
}

int CardTongHelper::getStrongestRank(vector<int> cardIds)
{
	vector<int> ranks = getAllRanks(cardIds);
	return (*(max_element(ranks.begin(), ranks.end())));
}
int CardTongHelper::getStrongestSuit(vector<int> cardIds)
{
	vector<int> suits = getAllSuits(cardIds);
	return (*(max_element(suits.begin(), suits.end())));
}

vector<int> CardTongHelper::getCardsWithSameRank(int rank, vector<int> cardIds)
{
...
vector<int> CardTongHelper::getCardsWithSameSuit(int suit, vector<int> cardIds)
{
	vector<int> resultIds;
	int id; 
	int foundSuit;
...
vector<int> CardTongHelper::getAllCardIds()
{
	vector<int> vCard;
	int rank;
	int suit;
...
vector<int> CardTongHelper::getAllRanks(vector<int> cardIds)
{
...
vector<int> CardTongHelper::getAllSuits(vector<int> cardIds)
{
...
int CardTongHelper::getNextRank(int rank)
{
	if (TongitsCard::RANK_ACE == rank) return TongitsCard::RANK_2;
	return (++rank);
}
int CardTongHelper::getPrevRank(int rank)
{
	if (TongitsCard::RANK_2 == rank) return TongitsCard::RANK_ACE;
	return (--rank);
}

bool CardTongHelper::hasRank(int rank, vector<int> cardIds)
{
	vector<int> ranks = getAllRanks(cardIds);
...
bool CardTongHelper::hasSuit(int suit, vector<int> cardIds)
{
	vector<int> suits = getAllSuits(cardIds);
...
bool CardTongHelper::validRank(int rank)
{
	return ((TongitsCard::RANK_2 <= rank && rank <= TongitsCard::RANK_ACE) ? true : false);
}
bool CardTongHelper::validSuit(int suit)
{
	return ((TongitsCard::SUIT_CLUB <= suit && suit <= TongitsCard::SUIT_SPADE) ? true : false);
}

int CardTongHelper::getComplexHandLevel(int type)
{
...
int CardTongHelper::getHandLevel(int type)
{
...
int CardTongHelper::getStrongestCardExcept2(vector<int> cardIds)
{
	vector<int> cards = cardIds;
	int invalidCard;
	for (int i = 0, l = cards.size(); i < l; i++)
	{
		if (getRank(cards.at(i)) == TongitsCard::RANK_2)
		{
...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
int CardTongHelper::getStrongerCard(int cardId, vector<int> cardIds){ return 0; }
int CardTongHelper::getStrongestCard(vector<int> cardIds)
{
...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
CardHand* CardTongHelper::getStrongerHand(CardHand* hand, vector<int> cardIds, vector<int> requiredCardIds)
{
...
*/
inline CardHand* CardTongHelper::getStrongerComplexHand(CardHand* hand, vector<int> cardIds, vector<int> requiredCardIds)
{ 
	int type;
	CardHand* foundHand = nullptr;
...
inline CardHand* CardTongHelper::getStrongerComplexHandByType(int type, CardHand* hand, vector<int> cardIds, vector<int> requiredCardIds)
{
	CardHand* foundHand = nullptr;
...
inline CardHand* CardTongHelper::getStrongerPairHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds)
{
...
inline CardHand* CardTongHelper::getStrongerTwoPairHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds)
{
	return nullptr;
}
inline CardHand* CardTongHelper::getStrongerThreeOfAKindHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds)
{
	int orderRank = getRank(hand->getCardIds().at(0));
	int checkingRank;
	vector<int> allRanks;
...
inline CardHand* CardTongHelper::getStrongerStraightHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds)
{
	...
	if (hasRank(TongitsCard::RANK_2, attackedCards) && hasRank(TongitsCard::RANK_6, attackedCards))
	{
		bStrongestStraight = true;
		strongestAttackedCard = getCardsWithSameRank(TongitsCard::RANK_6, attackedCards).at(0);
	}
	// Weakest SSCard, rank 5 is the king
	else if (hasRank(TongitsCard::RANK_2, attackedCards) && hasRank(TongitsCard::RANK_5, attackedCards))
	{
		bWeakestStraight = true;
		strongestAttackedCard = getCardsWithSameRank(TongitsCard::RANK_5, attackedCards).at(0);
	}	
		...
		if (curRank == wantedRank)
		{
			...
			if (vFoundRanks.size() == 5)
			{
				...
				if (bStrongestStraight)
				{
					...
					if (std::find(vFoundRanks.begin(), vFoundRanks.end(), TongitsCard::RANK_2) == vFoundRanks.end()
						|| std::find(vFoundRanks.begin(), vFoundRanks.end(), TongitsCard::RANK_6) == vFoundRanks.end()
						|| getStrongestCardWithRank(TongitsCard::RANK_6, allCardIds) > strongestAttackedCard)
					{
						...
						for (int i = 0, l = vFoundRanks.size(); i < l; i++)
						{
							...
							if (vFoundRanks.at(i) == TongitsCard::RANK_6) wantedCards.push_back(*(max_element(foundCards.begin(), foundCards.end())));
				...
				else if (bWeakestStraight)
				{
					// We have weaker hand
					if (std::find(vFoundRanks.begin(), vFoundRanks.end(), TongitsCard::RANK_2) == vFoundRanks.end()
						|| std::find(vFoundRanks.begin(), vFoundRanks.end(), TongitsCard::RANK_5) == vFoundRanks.end()
						|| getStrongestCardWithRank(TongitsCard::RANK_5, allCardIds) < strongestAttackedCard)
					{
						vFoundRanks.erase(vFoundRanks.begin(), vFoundRanks.begin() + 1);
						beginRankStraight = vFoundRanks.at(0);
					}
					...
					else
					{
						...
						for (int i = 0, l = vFoundRanks.size(); i < l; i++)
						{
							...
							if (vFoundRanks.at(i) == TongitsCard::RANK_5) wantedCards.push_back(*(max_element(foundCards.begin(), foundCards.end())));
							else wantedCards.push_back(foundCards.at(0));
						}
						foundHand = new CardHand(CardHand::STRAIGHT, wantedCards);
						break;
				...
				else
				{
					...
					vector<int>::iterator it = std::find(vTmpRank.begin(), vTmpRank.end(), TongitsCard::RANK_2);
...
inline CardHand* CardTongHelper::getStrongerFlushHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds )
{
	vector<int> attackedCards = hand->getCardIds();
...
inline CardHand* CardTongHelper::getStrongerFullhouseHand(CardHand* hand, vector<int> allCardIds, vector<int> requiredCardIds)
{
	CardHand* foundHand = nullptr;
...
inline CardHand* CardTongHelper::getStrongerFourOfAKindHand(CardHand* hand, vector<int> cardIds, vector<int> requiredCardIds ){return nullptr;}
inline CardHand* CardTongHelper::getStrongerStraightFlushHand(CardHand* hand, vector<int> cardIds, vector<int> requiredCardIds ){return nullptr;}

CardHand* CardTongHelper::getHand(vector<int> cardIds)
{
...
CardHand* CardTongHelper::getSusunHand(vector<int> cardIds)
{
	CardHand* foundHand = getStraightFlushHand(cardIds);
...
CardHand* CardTongHelper::getPairHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
...
CardHand* CardTongHelper::getTwoPairHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	vector<CardHand*> vHand = getAllTwoPairHands(allCardIds);
	return (vHand.size() > 0 ? vHand.at(0) : nullptr);
}

CardHand* CardTongHelper::getThreeOfAKindHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	int noCard = allCardIds.size();
...
CardHand* CardTongHelper::getFourOfAKindHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	int noCard = allCardIds.size();
...
CardHand* CardTongHelper::getStraightHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	vector<CardHand*> vHand = getAllStraightHands(allCardIds, nullptr);
	if (vHand.size() > 0) return vHand.at(0);
	else return nullptr;
}
CardHand* CardTongHelper::getFlushHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	vector<CardHand*> vHand = getAllFlushHands(allCardIds);
	return (vHand.size() > 0 ? vHand.at(0) : nullptr);

}
CardHand* CardTongHelper::getFullHouseHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	CardHand* foundHand = nullptr;
...
CardHand* CardTongHelper::getStraightFlushHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
...
vector<CardHand*> CardTongHelper::getAllPairHands(vector<int> allCardIds)
{
...
vector<CardHand*> CardTongHelper::getAllThreeOfAKindHands(vector<int> allCardIds)
{
	vector<CardHand*> vHand = {};
...
vector<CardHand*> CardTongHelper::getAllTwoPairHands(vector<int> allCardIds)
{
	vector<CardHand*> vHand = {};
...
vector<CardHand*> CardTongHelper::getAllFlushHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
	...
	for (int i = 0, l = checkingSuits.size(); i < l; i++)
	{
		...
		{
			std::sort(cardsWithSuit.begin(), cardsWithSuit.end(), sortIncreaseTong);
...
vector<CardHand*> CardTongHelper::getAllStraightHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
	...
	CardHand* foundHand = nullptr;
	std::sort(allValidRanks.begin(), allValidRanks.end(), sortIncreaseTong);
	int beginRankDeck = allValidRanks.at(0);
	int beginRankStraight = allValidRanks.at(0);
	int lastRank = beginRankDeck;
	int wantedRank;
	...
	while (curRank != TongitsCard::RANK_ACE && count < allValidRanks.size()-1)
	{
		count++;
		curRank = allValidRanks.at(count);
		wantedRank = getNextRank(lastRank);
		...
		if (curRank == wantedRank)
		{
			...
			if (vFoundRanks.size() == 5)
			{
				...
				vector<vector<int>> vCombination = getStraightCardsFromRanksTong(vFoundRanks, allCardIds);
	...
	if (attackerLv == straightLv)
	{
		...
		for (i = 0, li = vHand.size(); i < li; i++)
		{
			...
			if (hasRank(TongitsCard::RANK_ACE, foundCards) && hasRank(TongitsCard::RANK_5, foundCards))
			{
				strongestHandCard = getCardsWithSameRank(TongitsCard::RANK_5, foundCards).at(0);
				if (bSmallest && strongestHandCard > strongestAttackerCard) break;
			}
			else if (hasRank(TongitsCard::RANK_2, foundCards) && hasRank(TongitsCard::RANK_6, foundCards))
			{
				strongestHandCard = getCardsWithSameRank(TongitsCard::RANK_6, foundCards).at(0);
    ...
    if (std::find(allValidRanks.begin(), allValidRanks.end(), TongitsCard::RANK_ACE) != allValidRanks.end()
        && std::find(allValidRanks.begin(), allValidRanks.end(), TongitsCard::RANK_2) != allValidRanks.end()
        && std::find(allValidRanks.begin(), allValidRanks.end(), TongitsCard::RANK_3) != allValidRanks.end()
        && std::find(allValidRanks.begin(), allValidRanks.end(), TongitsCard::RANK_4) != allValidRanks.end()
        && std::find(allValidRanks.begin(), allValidRanks.end(), TongitsCard::RANK_5) != allValidRanks.end())
    {
        vector<int> specialCards = {};
        specialCards.push_back(getCardsWithSameRank(TongitsCard::RANK_ACE, allCardIds).at(0));
        specialCards.push_back(getCardsWithSameRank(TongitsCard::RANK_2, allCardIds).at(0));
        specialCards.push_back(getCardsWithSameRank(TongitsCard::RANK_3, allCardIds).at(0));
        specialCards.push_back(getCardsWithSameRank(TongitsCard::RANK_4, allCardIds).at(0));
        specialCards.push_back(getCardsWithSameRank(TongitsCard::RANK_5, allCardIds).at(0));
        vHand.push_back(new CardHand(CardHand::STRAIGHT, specialCards));
    }
	return vHand;
}

vector<CardHand*> CardTongHelper::getAllFullhouseHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
...
vector<CardHand*> CardTongHelper::getAllFourOfAKindHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
...
vector<CardHand*> CardTongHelper::getAllStraightFlushHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
...
CardHand* CardTongHelper::getExactFourOfAKindHand(vector<int> allCardIds, vector<int> requiredCardIds)
{
	int noCard = allCardIds.size();
...
vector<CardHand*> CardTongHelper::getAllExactFourOfAKindHands(vector<int> allCardIds, CardHand* attackerHand)
{
	vector<CardHand*> vHand = {};
...
bool CardTongHelper::compare(CardHand* handA, CardHand* handB)
{
...
bool CardTongHelper::compareHighHands(vector<int> cardsA, vector<int> cardsB)
{
...
// ---------------------------------------------------------
// ---------------------------------------------------------
void CardTongHelper::sortByHand(vector<int> &cardIds)
{
...
void CardTongHelper::sortByUpRank(vector<int> &cardIds)
{
	std::sort(cardIds.begin(), cardIds.end(), sortIncreaseTong);
}

void CardTongHelper::sortByDownRank(vector<int> &cardIds)
{

}

void CardTongHelper::sortBySuit(vector<int> &cardIds)
{
	vector<int> cardsWithDiamondSuit = getCardsWithSameSuit(TongitsCard::SUIT_DIAMOND, cardIds);
	vector<int> cardsWithClubSuit = getCardsWithSameSuit(TongitsCard::SUIT_CLUB, cardIds);
	vector<int> cardsWithHeartSuit = getCardsWithSameSuit(TongitsCard::SUIT_HEART, cardIds);
	vector<int> cardsWithSpadeSuit = getCardsWithSameSuit(TongitsCard::SUIT_SPADE, cardIds);
	cardIds.clear();
...
bool CardTongHelper::checkDragonSameSuit(vector<int> allCardIds)
{
	if (getAllSuits(allCardIds).size() != 1) return false;
	if (getAllRanks(allCardIds).size() != 13) return false;
	return true;
}
bool CardTongHelper::checkDragon(vector<int> allCardIds)
{
	if (getAllRanks(allCardIds).size() != 13) return false;
	return true;
}
bool CardTongHelper::checkSameSuit(vector<int> allCardIds)
{
	vector<int> allSuits = getAllSuits(allCardIds);
...
bool CardTongHelper::checkFivePairsOneTriple(vector<int> allCardIds)
{
	vector<int> allRanks = getAllRanks(allCardIds);
	int countPair = 0;
...
bool CardTongHelper::checkSixPairs(vector<int> allCardIds)
{
	vector<int> allRanks = getAllRanks(allCardIds);
	int countPair = 0;
	for (int i = 0, l = allRanks.size(); i < l; i++) {
...
bool CardTongHelper::checkThreeFlushes(vector<int> allCardIds)
{
...
bool CardTongHelper::checkThreeStraights(vector<int> allCardIds)
{
	vector<int> firstHandCards(3);
	vector<int> secondHandCards(5);
	vector<int> thirdHandCards(5);
	...
	if ((std::find(firstRanks.begin(), firstRanks.end(), TongitsCard::RANK_ACE) != firstRanks.end()
		 && std::find(firstRanks.begin(), firstRanks.end(), TongitsCard::RANK_2) != firstRanks.end()
		 && std::find(firstRanks.begin(), firstRanks.end(), TongitsCard::RANK_3) != firstRanks.end())
...
void CardTongHelper::sortByStraight(vector<int> &cardIds)
{
...
inline void CardTongHelper::sortByStraightHand(vector<int> &cardIds){}
inline void CardTongHelper::sortByFlushHand(vector<int> &cardIds){}
inline void CardTongHelper::sortByFullHouseHand(vector<int> &cardIds){}
inline void CardTongHelper::sortByFourOfAKindHand(vector<int> &cardIds){}
inline void CardTongHelper::sortByStraighFlushHand(vector<int> &cardIds){}
