...
void EndGamePopup::fillData()
{
	...
	for (int i = 0; i < si; i++)
	{
        ...
        if (data->m_bwin==false)
        {
            //remove symbol -
            string str(ccsf("%1.0f",data->m_dChangeMoney));
            ...
            str=TTutil::addDot(str);
            std::string strMoney="";
            strMoney.append("-");
            strMoney.append(str);
            txtMoney->setString(strMoney);
        }
        else
        {
...
void EndGamePopup::fillDataSS()
{
	...
	for (int i = 0; i < si; i++)
	{
        ...
        if (data->m_dMoneyChanged<0)
        {
            //remove symbol -
            string str(ccsf("%1.0f",data->m_dMoneyChanged));
