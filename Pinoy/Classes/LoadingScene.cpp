...
void LoadingScene::onEnter()
{
    ...
    relayout();
    //init smartfox server
#if CC_TARGET_PLATFORM == CC_PLATFORM_WP8
    //g_smartfoxClientCallback->initClient();
#else
...
void LoadingScene::loadResource()
{
    ...
    //load lang
    ResourceManager::getInstance()->loadLanguagesData();
...
void LoadingScene::update(float dt)
{
    //BaseScene::update(dt);
    m_iFrameCount++;
    if(ResourceManager::getInstance()->m_bLoadConfigFinish && GlobalVariable::m_bResourceLoaded)
    {
