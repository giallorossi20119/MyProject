...
//GlobalVariable::m_pSettingCom->setVisibleItems(true, true);
//
...
//    }
    if(GlobalVariable::m_bWasBonusStarup)
    {
        ...
        AlertPopup* alp = AlertPopup::create("Pusoy Pinoy present you 50.000 Gold for startup!");
        openPopup(alp);
    }
    if(GlobalVariable::m_bOutofMoney)
    {
        GlobalVariable::m_bOutofMoney=false;
        AlertPopup* alp = AlertPopup::create("You're out of Gold.Please Purchase items to have more Gold to play!"
);
        openPopup(alp);
    }
}
...
void LobbyScene::getZone(Ref *pSender, Widget::TouchEventType type)
{
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        {
            if(i==1 && GlobalVariable::m_pUser->m_dMoney<100000)
            {
                openPopup(new AlertPopup("You need at least 100.000 Gold to join this room"));
                GlobalVariable::m_iCurrentZone = 1;
            }
            else if (i==2 && GlobalVariable::m_pUser->m_dMoney<1000000)
            {
                openPopup(new AlertPopup("You need at least 1.000.000 Gold to join this room"));
                GlobalVariable::m_iCurrentZone = 1;
            }
            else
            {
