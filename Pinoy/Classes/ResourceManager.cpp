...
void ResourceManager::loadLanguagesData()
{
	m_requestLang = new HttpRequest();
	m_requestLang->setRequestType(HttpRequest::Type::GET);
	m_requestLang->setResponseCallback(CC_CALLBACK_2(ResourceManager::onResponseLang, this));
    //string strLanguageURL = kBaseURL;
    string strLanguageURL = "http://gcdn.gameloe.com/susun/Languages.txt";
    //strLanguageURL.append(kLanguageFile);
	m_requestLang->setUrl(strLanguageURL.c_str());
	HttpClient::getInstance()->sendImmediate(m_requestLang);
	m_requestLang->release();
}
void ResourceManager::onResponseLang(HttpClient* sender, HttpResponse* response)
{
	...
	GlobalVariable::langData.Parse<0>(l_s_data.c_str());
}
...
void ResourceManager::loadTextureData()
{
    ...
    Director::getInstance()->getTextureCache()->addImage("theblack_overlay.png");
...
const string strSpine[kNOAnims] = {"girl", "trang", "thang", "vebet", "moiban", "good", "bad", "sangvanmoi", "Timeline", "card_swap", "exp", "smke"};
//const string strSpine[kNOAnims] = {"trang", "thang", "vebet", "moiban", "good", "bad", "sangvanmoi", "Timeline", "card_swap", "exp", "smke"};
