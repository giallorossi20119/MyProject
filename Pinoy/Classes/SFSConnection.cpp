...
void SFSConnection::ping()
{
    ...
    else if(GlobalVariable::m_iCurrentGame == 2)
    {
        boost::shared_ptr<IRequest> request (new ExtensionRequest(T_CLIENT_PING, NULL));
        mSmartFox->Send( request );
    }
...
void SFSConnection::startGame()
{
    ...
    {
        boost::shared_ptr<IRequest> request(new ExtensionRequest(T_START_GAME, NULL));
        mSmartFox->Send(request);
    }
    else
    {
...
void SFSConnection::fireCard(boost::shared_ptr<ByteArray> bytesCards)
{
    ...
    {
        boost::shared_ptr<IRequest> request(new ExtensionRequest(T_DANH_BAI, parameters));
        mSmartFox->Send(request);
    }
...
//pick card
void SFSConnection::pickCard()
{
    CCLOG("SFSConnection: send pickCard start");
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_GET_CARD_FROM_DECK, NULL));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: send pickCard end");
}

//drop card
void SFSConnection::dropCard(boost::shared_ptr<ByteArray> bytesCards)
{
    CCLOG("SFSConnection: send dropCard start");
    boost::shared_ptr<ISFSObject> parameters (new SFSObject());
    parameters->PutByteArray("d",bytesCards);
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_HA_BAI, parameters));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: send dropCard end");
}

//eat card
void SFSConnection::eatCard(boost::shared_ptr<ByteArray> bytesCards)
{
    CCLOG("SFSConnection: send eatCard start");
    boost::shared_ptr<ISFSObject> parameters (new SFSObject());
    parameters->PutByteArray("d",bytesCards);
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_AN_BAI, parameters));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: send eatCard end");
}

//send card
void SFSConnection::sendCard(boost::shared_ptr<ByteArray> byteCards)
{
    CCLOG("SFSConnection: send Card start");
    boost::shared_ptr<ISFSObject> parameters (new SFSObject());
    parameters->PutByteArray("d",byteCards);
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_AUTO_GUI_BAI, parameters));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: send Card end");
}

//request u
void SFSConnection::requestU()
{
    CCLOG("SFSConnection: request U start");
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_U, NULL));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: request U end");
}

//send draw confirm
void SFSConnection::requestDrawConfirm(bool accept)
{
    CCLOG("SFSConnection: send draw confirm start");
    boost::shared_ptr<ISFSObject> parameters(new SFSObject());
    boost::shared_ptr<ByteArray> btr (new ByteArray());
    btr->WriteBool(accept);
    parameters->PutByteArray("d",btr);
    //send request
    boost::shared_ptr<IRequest> request(new ExtensionRequest(T_XACTHUC_BAO_BAI, parameters));
    mSmartFox->Send(request);
    CCLOG("SFSConnection: send draw confirm end");
}

//request draw
void SFSConnection::requestDraw()
{
    CCLOG("SFSConnection: send draw start");
    boost::shared_ptr<IRequest> request (new ExtensionRequest(T_BAO_BAI, NULL));
    mSmartFox->Send( request );
    CCLOG("SFSConnection: send draw end");
}
...
void SFSConnection::requestChangeSettingRoom(int stake, int times)
{
    ...
    else if(GlobalVariable::m_iCurrentGame == 2)
    {
        ...
        bytes->WriteInt(times);
        parameters->PutByteArray("d", bytes);
        boost::shared_ptr<IRequest> request(new ExtensionRequest(T_CHANGE_SETTING_GAME, parameters));
        mSmartFox->Send(request);
    }
    else //susun
    {
...
void SFSConnection::kickUser(long iKickedUserId, double dKickedUserId)
{
    ...
    else if(GlobalVariable::m_iCurrentGame == 2)
    {
        ...
        btr->WriteDouble(dKickedUserId);
        parameters->PutByteArray("d", btr);
        boost::shared_ptr<IRequest> request(new ExtensionRequest(T_KICKUSER, parameters));
        mSmartFox->Send(request);
    }
    else
    {
